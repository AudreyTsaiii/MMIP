...
def process_folder(input_dir, output_dir, debug=False, manual_fallback=False):
    ...
    for path in sorted(image_paths):
        ...
        corners, edged = find_document_corners(
            image,
            debug=debug
        )
        ...
        if corners is None:
            ...
            print(f"[Contour 失敗] {filename}")
            print("   -> 嘗試使用 HoughLinesP...")

            corners, hough_debug = find_corners_by_hough(
                image,
                debug=debug
            )
            ...
            if corners is not None:
                ...
                print(f"[Hough 成功] {filename}")

            else:
                print(f"[Hough 也失敗] {filename}")
...
if __name__ == "__main__":
    ...
    MANUAL_FALLBACK = False        # True: 自動偵測失敗時跳出視窗讓你手動點4個角點
